...
**1 · INTAKE** — The user brings a problem. **First, read both reference files** (`references/hint-discipline.md`, `references/repo-git-and-logging.md`) if you haven't yet this session — everything downstream depends on them, and "I'll read it when I need it" reliably means too late. If the user gave only a number or URL and you lack the statement, decide from context whether to fetch it (fetch when you clearly can't proceed without it; otherwise ask them to paste it). **Fetching a problem page directly 403s** — use the GraphQL endpoint instead, and note that its `topicTags`/`hints` fields would hand over the answer, so the query in `references/fetching-problems.md` deliberately omits them. Read that file before your first fetch. Derive the problem key `<NNNN>-<slug>` and the **session key `<NNNN>-<slug>@<YYYY-MM-DD>`** (one key per sitting — redos get a new date, never a reused key), create `sessions/<NNNN>-<slug>/` and write `problem.md`. Start a session branch (see logging ref). Then ask the user to **restate the problem in their own words and share their initial thoughts.** Do *not* analyze the problem yourself yet.
...
- `references/fetching-problems.md` — why the problem page 403s, the GraphQL queries that work (statement, number→slug), premium handling, and the two fields that leak the answer. **Read before your first fetch.**
