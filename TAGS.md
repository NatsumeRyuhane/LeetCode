...
- `#technique:amortized-analysis` — bound the *total* work of a run by charging each unit of it to a resource consumed exactly once (an element is pushed once, therefore popped at most once), instead of multiplying a worst-case single operation by the number of operations. The signature situation: one call is genuinely expensive and you can build the input proving it, but a second expensive call cannot occur until enough cheap work has paid for it. Distinct from the memoization bound (states × per-state cost), where each call is cheap and it is the *count* that needs bounding; the aggregate form is stronger because it survives when no individual call is cheap
- `#technique:index-as-key` — store the index into the source array, not a tuple of quantities derived from it. If the window width is fixed then both the value (`nums[j]`) and the expiry (`j + k`) are functions of `j`, so caching them buys no information and costs an allocation per element; the derived comparisons rearrange into index comparisons (`q[0] < i-k+1` ⟺ `i >= j+k`). General check: before storing a tuple, ask whether one component determines the rest through data already in hand
...
- `#technique:invariant-repair` — when an input needs a special case, first ask whether some structure's *stated meaning* has quietly become false for it. If `S` means "every value that could be probed" and one input leaves a value out of `S`, the fix is to put it in, not to branch around the consequence: restoring the invariant deletes the branch, while patching the return value preserves it plus whatever premise it smuggled in. Diagnostic: write the container's meaning as a sentence, then find the input for which the sentence is a lie. Companion at loop level — a loop has a *declared* bound (large enough not to cut the search short) and an *operative* bound (what actually governs the cost); deriving the declared bound from the operative one often dissolves the empty-range special case too
- `#technique:dp-with-multiplicity` — when the dp already computes an optimum, count *how many ways* achieve it by carrying a second accumulator in the same cell and propagating it by the same recurrence: on a strictly better child, **overwrite** the counter with that child's count; on a tie, **add** it. The counter never increments by 1 — a predecessor contributes its own multiplicity, not its existence — and the seed is 1 (the one-element solution), never 0, or the whole table zeroes out. The answer is then the *sum* over every cell attaining the optimum, not the value at one cell. Diagnostic that the pairing is right: "count of predecessors" and "sum of predecessors' counts" agree on every input where all counts are 1, so a test suite must contain a cell whose count exceeds 1 *and* is consumed by a later cell
...
- `#weakness:coverage-without-discrimination` — the successor to `#weakness:confirmatory-testing`, and invisible to the diagnostic that catches it: every branch has a test and every line runs, but no test runs the load-bearing line *in a state where being wrong would change the answer*. Signature shape — an accumulator whose values are all 1 on the tested inputs, so `+= x` and `+= 1` are the same program; or a reset that never discards anything, so `= x` and `= 1` agree. Coverage measures which lines executed, not whether the execution could have distinguished a correct line from a wrong one. Diagnostic (mutation testing): pick the line the whole method rests on, change one token to something plausible-but-wrong, and re-run the suite — a mutant that survives is a test you have not written yet
