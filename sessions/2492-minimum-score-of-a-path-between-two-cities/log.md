...
**Redo note (2026-07-04, post-debrief).** The editorial lists a third method: minimum-spanning-tree via Prim. Not studied — user was done for the day. On a redo of 2492: start from the claim "the component's min edge necessarily appears in the MST" and derive *why* the MST greedy property guarantees it, then implement the Prim variant and compare all three (BFS / DSU / Prim) on cost and code shape. Pairs naturally with the path-compression item above.
